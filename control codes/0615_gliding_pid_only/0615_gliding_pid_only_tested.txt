...
/*
  ...
  Assumptions:
    ...
    - Servo 1 signal D4, servo 2 signal D3.
...
static const char LOG_TAG[] = "0615";
...
static const float SERVO_LEFT_NEUTRAL_DEG  = 95.0f; // raw servo deg for LEFT  wing 0 incidence 
static const float SERVO_RIGHT_NEUTRAL_DEG = 95.0f; // raw servo deg for RIGHT wing 0 incidence

static const float AOA_RATE_LIMIT_DEG_PER_S= 120.0f;
static const float AOA_CMD_LIMIT_DEG       = 0.0f;
...
// PID gains
static const float KP_ROLL  = 0.50f;
...
struct ControlRecord {
  ...
  float leftCmdDeg, rightCmdDeg;
  float leftServoDeg, rightServoDeg;
  uint16_t leftPwmUs, rightPwmUs;
};

// ==================================== Inter-Core Queue ====================================
//
// Queue depth: 20 records × sizeof(ControlRecord) ≈ 20 × 88 B = 1.76 KB of RAM.
...
static void writeCsvHeader(Print &out) {
  out.println(F("ms,ax_g,ay_g,az_g,a_norm_g,gx_dps,gy_dps,gz_dps,"
                ...
                "left_cmd_deg,right_cmd_deg,"
                "left_servo_deg,right_servo_deg,"
                "left_pwm,right_pwm"));
}
...
//   return reverse ? SERVO_MIN_DEG + SERVO_MAX_DEG - deg : deg;
// }
// static float scaleServoTravel(float deg, float gain) {
//   return clampfLocal(AOA_NEUTRAL_DEG + (deg - AOA_NEUTRAL_DEG) * gain,
...
//   leftServoDeg  = rateLimit(scaleServoTravel(leftDeg,  SERVO_LEFT_GAIN),  leftServoDeg,  step);
//   rightServoDeg = rateLimit(scaleServoTravel(rightDeg, SERVO_RIGHT_GAIN), rightServoDeg, step);
...
static void updatePidAndServos(float dt) {
  if (!estimatorReady) {
    leftCmdDeg = rightCmdDeg = leftCmdFiltDeg = rightCmdFiltDeg = 0.0f;
  ...
  uRollDeg  = clampfLocal( KP_ROLL  * rollErr  + KI_ROLL  * rollI  + KD_ROLL  * pRate,
                           -AOA_CMD_LIMIT_DEG, AOA_CMD_LIMIT_DEG);
  ...
  writeServos(AOA_NEUTRAL_DEG + leftCmdFiltDeg,
              AOA_NEUTRAL_DEG + rightCmdFiltDeg, dt);
}
