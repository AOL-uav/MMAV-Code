...
static const float SERVO_CMD_LIMIT_DEG = 25.0f;
static const float SERVO_RATE_LIMIT_DEG_PER_S = 200.0f;
static const float SERVO_COMMAND_LPF_ALPHA = 0.45f;
...
static const float SMC_LAMBDA_ROLL = 3.0f;
static const float SMC_LAMBDA_PITCH = 3.0f;
static const float SMC_K_ROLL_RADPS2 = 4.0f;
static const float SMC_K_PITCH_RADPS2 = 4.0f;
static const float SMC_PHI_ROLL_RADPS = 0.18f;
static const float SMC_PHI_PITCH_RADPS = 0.18f;
...
static float leftServoDeg = SERVO_NEUTRAL_DEG;
static float rightServoDeg = SERVO_NEUTRAL_DEG;
...
static void clearControlState() {
  ...
  leftServoDeg = SERVO_NEUTRAL_DEG;
  rightServoDeg = SERVO_NEUTRAL_DEG;
...
static void updateSmc() {
  ...
  const float rollError = wrapPi(rollRad - ROLL_TARGET_RAD);
  const float pitchError = wrapPi(pitchRad - PITCH_TARGET_RAD);

  sRoll = pRate + SMC_LAMBDA_ROLL * rollError;
  sPitch = qRate + SMC_LAMBDA_PITCH * pitchError;

  // Desired generalized angular acceleration. sat() creates a boundary layer
  // around the sliding surface and avoids sign-function servo chatter.
  const float desiredAlpha[2] = {
    -SMC_LAMBDA_ROLL * pRate
        - SMC_K_ROLL_RADPS2 * sat(sRoll / SMC_PHI_ROLL_RADPS)
        - matchedDisturbance[0],
    -SMC_LAMBDA_PITCH * qRate
...
static void writeServos(float dt) {
  ...
  leftPwmUs =
      (uint16_t)pwmFromServoDegrees(leftServoDeg, SERVO_LEFT_REVERSE);
  rightPwmUs =
      (uint16_t)pwmFromServoDegrees(rightServoDeg, SERVO_RIGHT_REVERSE);
  ...
  previousControlRad[0] =
      0.5f * (leftServoDeg - rightServoDeg) * DEG_TO_RAD_F;
  previousControlRad[1] =
      (0.5f * (leftServoDeg + rightServoDeg) - SERVO_NEUTRAL_DEG) *
      DEG_TO_RAD_F;
}
...
static void writeCsvHeader(Print &out) {
  out.println(F(
      ...
      "s_roll,s_pitch,u_diff_deg,u_sym_deg,"
      "b00,b01,b10,b11,left_deg,right_deg,left_pwm,right_pwm,"
...
static void printFrequencyStatus(
  ...
  Serial.println(esekfResetCount);
  serialMutex.unlock();
}
...
static void loggerTask() {
  ...
  if (sdReady) {
    char filename[16];
    for (int i = 0; i < 100; i++) {
      snprintf(filename, sizeof(filename), "P613_%02d.CSV", i);
...
void setup() {
  ...
  serialMutex.lock();
  Serial.println(F("70 ESEKF + adaptive-B SMC boot"));
  Serial.println(F(
      "GPS optional: NEO-6M UBX POSLLH/SOL/VELNED, Serial1 9600"));
  serialMutex.unlock();

  if (!IMU.begin()) {
    serialMutex.lock();
    Serial.println(F("FATAL: onboard LSM6DSOX IMU not detected"));
    serialMutex.unlock();
  ...
  servoLeft.writeMicroseconds(
      pwmFromServoDegrees(SERVO_NEUTRAL_DEG, SERVO_LEFT_REVERSE));
  servoRight.writeMicroseconds(
      pwmFromServoDegrees(SERVO_NEUTRAL_DEG, SERVO_RIGHT_REVERSE));
  ...
  if (!calibrateGyro()) {
    serialMutex.lock();
    Serial.println(F("FATAL: IMU gyro calibration failed"));
    serialMutex.unlock();
    while (true) delay(1000);
  }
...
void loop() {
  ...
  if (esekfHealthy()) {
    if (esekfSettleRemaining > 0) {
      ...
      uDiffFilteredRad = 0.0f;
      uSymFilteredRad = 0.0f;
    } else if (armed && !esekfFaultLocked) {
