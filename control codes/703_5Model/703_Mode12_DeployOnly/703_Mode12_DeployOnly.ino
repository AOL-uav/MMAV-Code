...
/*
  703_Mode12_DeployOnly

  Deployment-only test build: stays in M2 after the wings open and
  never enters M3, so the SMC stays inactive.
...
static void updateFlightMode(uint32_t nowUs) {
  ...
  if (flightMode == MODE_CARRIED_FOLDED ||
      flightMode == MODE_FAILSAFE ||
      modeStartUs == 0) {
    // Mode12: re-arm after deployment re-runs the deployment ramp
    // (this test build never enters M3).
    setFlightMode(
        missionDeployStarted
            ? MODE_DEPLOYMENT_TRANSIENT
            : MODE_FOLDED_ACCELERATION,
        nowUs);
  }
...
void setup() {
  ...
  serialMutex.lock();
  Serial.print(F("[Core 0] 703 Mode12 DeployOnly boot, log tag="));
