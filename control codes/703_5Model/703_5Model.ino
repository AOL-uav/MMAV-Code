...
static const char LOG_TAG[] = "0703";
...
static void updateFlightMode(uint32_t nowUs) {
  ...
  if (flightMode == MODE_CARRIED_FOLDED ||
      flightMode == MODE_FAILSAFE ||
      modeStartUs == 0) {
    // Fresh arm starts the folded drop; re-arm after deployment resumes
    // the glide directly because the wings are already open.
    setFlightMode(
...
void setup() {
  ...
  serialMutex.lock();
  Serial.print(F("[Core 0] 703 5Model boot, log tag="));
