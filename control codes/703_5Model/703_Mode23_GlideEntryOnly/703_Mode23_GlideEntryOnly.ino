...
// ESEKF_FAULT_AUTO_RECOVER 1: after an ESEKF fault the filter is
// re-initialized; if it stays healthy through the settle window the fault
// latch is released and the (still low) ARM input re-enables control
// about one second later. Set 0 to keep the 626 behavior where a single
// ESEKF fault disables control until power cycle.
#define ESEKF_FAULT_AUTO_RECOVER 1
...
/*
  703_Mode23_GlideEntryOnly

  Glide-entry test build: arming starts at the deployment transient
  (M2) and skips the folded-acceleration phase M1.

  Changes from 626_5Model:
    - In-air disarm/fault no longer refolds the wings: once deployment
      has started, disarm enters MODE_FAILSAFE and holds the morph servo;
      re-arm resumes M3 directly instead of re-running M1/M2.
    - Mission altitude model in gliding modes uses a sink fraction of
      airspeed instead of the full airspeed magnitude.
    - D2->3 timer and the M1 altitude backoff now include T2_MARGIN_S,
      and H1_LAST_M was raised to stay above the normal-window floor.
    - Gyro calibration retries instead of halting on a single bump.
    - ESEKF fault latch auto-recovers after a healthy settle window
      (ESEKF_FAULT_AUTO_RECOVER, set 0 for the old latch-forever rule).
    - M2 deployment ramp starts from the current morph position, so a
      re-armed deployment never snaps the wings back to folded first.
    - SD logging continues after an IMU fault latch so the GPS descent
      is still recorded (rows with control_dt_us=0 mark that state).
...
// Keep H1_LAST_M >= H_OPEN_MIN_M + VZ_12_BAR_MPS * (T2_OPEN_S +
// T2_SETTLE_S + T2_MARGIN_S) = 30.5 m so the no-speed fallback fires
// before the normal deployment window closes.
static const float H1_LAST_M = 32.0f;
...
// Fraction of airspeed lost as sink while gliding (~4:1 glide ratio).
// PLACEHOLDER: measure from drop-test logs before trusting M4/M5
// altitude gates on this model.
static const float GLIDE_SINK_PER_AIRSPEED = 0.25f;
...
// Latched once M2 begins; survives disarm so a fault or ARM-wire loss
// in the air never commands the wings to refold.
static bool missionDeployStarted = false;
// Morph position captured when M2 begins so the deployment ramp starts
// from the actual wing position instead of snapping back to folded.
static float missionBetaRampStartDeg = BETA_FOLDED_DEG;
...
static void updateMissionStateEstimator(
  ...
  float descentRateModel;
  if (flightMode == MODE_DEPLOYMENT_TRANSIENT) {
    descentRateModel = VZ_12_BAR_MPS;
  } else if (flightMode == MODE_FOLDED_ACCELERATION) {
    // Folded fall is near-vertical: descent ~= speed.
    descentRateModel = vPrev;
  } else {
    // Gliding modes: only the sink component of airspeed reduces height.
    descentRateModel = GLIDE_SINK_PER_AIRSPEED * vPrev;
  }
  missionAltitudeEstM -=
...
static void setFlightMode(FlightMode nextMode, uint32_t nowUs) {
  ...
  // Never move backward while armed, except when leaving FAILSAFE on
  // re-arm.
  if (nextMode < flightMode && flightMode != MODE_FAILSAFE && armed) {
    return;
  }
  ...
  if (nextMode == MODE_DEPLOYMENT_TRANSIENT) {
    missionDeployStarted = true;
    missionBetaRampStartDeg = morphServoDeg;
  }
...
static void updateFlightMode(uint32_t nowUs) {
  ...
  if (!armed) {
    if (missionDeployStarted) {
      // Disarm after deployment began (ESEKF fault latch, ARM wire
      // loss, or post-landing disarm): NEVER command the wings to
      // refold in the air. Hold the morph servo at its last position
      // and mark FAILSAFE so the log shows why control stopped.
      flightMode = MODE_FAILSAFE;
      modeStartUs = 0;
      modeElapsedS = 0.0f;
  ...
  if (flightMode == MODE_CARRIED_FOLDED ||
      flightMode == MODE_FAILSAFE ||
      modeStartUs == 0) {
    // Mode23: fresh arm starts at the deployment transient and skips
    // M1; re-arm after deployment resumes the glide.
    setFlightMode(
        missionDeployStarted
            ? MODE_GLIDE_NAVIGATION
            : MODE_DEPLOYMENT_TRANSIENT,
        nowUs);
  }
  ...
  switch (flightMode) {
    case MODE_FOLDED_ACCELERATION: {
      ...
      const float T12 = T2_OPEN_S + T2_SETTLE_S + T2_MARGIN_S;
    ...
    case MODE_DEPLOYMENT_TRANSIENT: {
      ...
      missionBetaCmdDeg =
          missionBetaRampStartDeg +
          missionDeployProgress *
              (BETA_OPEN_DEG - missionBetaRampStartDeg);
      writeMorphServo(missionBetaCmdDeg);
      writeTailServo(SERVO_TAIL_TRIM_DEG);

      const float t2Min = T2_OPEN_S + T2_SETTLE_S + T2_MARGIN_S;
      ...
      if (missionD23) {
        ...
        writeMorphServo(missionBetaCmdDeg);
        setFlightMode(MODE_GLIDE_NAVIGATION, nowUs);
      }
      break;
    }
...
void setup() {
  ...
  serialMutex.lock();
  Serial.print(F("[Core 0] 703 Mode23 GlideEntryOnly boot, log tag="));
  ...
  for (uint8_t attempt = 0; attempt < 5 && !gyroCalibrated; attempt++) {
    gyroCalibrated = calibrateGyro();
    if (!gyroCalibrated) {
      safeSerialPrintln(
          F("[Core 0] Gyro calibration disturbed - keep still, retrying."));
      delay(500);
    }
  }
  if (!gyroCalibrated) {
...
void loop() {
  ...
  if (imuFaultLocked) {
    ...
    servoRight.writeMicroseconds(
        pwmFromServoDegrees(SERVO_RIGHT_NEUTRAL_DEG, false));
    writeTailServo(SERVO_TAIL_TRIM_DEG);
    // Keep recording GPS/servo state after an IMU fault so the descent
    // is still on the SD card for analysis and vehicle recovery.
...
#if ESEKF_FAULT_AUTO_RECOVER
      if (esekfSettleRemaining == 0 && esekfFaultLocked) {
        // The filter re-initialized and stayed healthy through the whole
        // settle window: release the latch so the ARM input can
        // re-enable control. esekf_resets in FREQ records the event.
        esekfFaultLocked = false;
      }
#endif
      eulerFromQuaternion(esekf.q, rollRad, pitchRad, yawRad);
      uDiffRad = 0.0f;
