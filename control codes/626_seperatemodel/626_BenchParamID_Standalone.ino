...
/*
  626_BenchParamID_Standalone

  Standalone bench/open-loop identification sketch.
  This file is intentionally separate from the full 626_5Model flight code.
...
static void writeServos() {
  servoLeft.writeMicroseconds(
      pwmFromServoDegrees(leftCmdDeg, SERVO_LEFT_REVERSE));
  servoRight.writeMicroseconds(
      pwmFromServoDegrees(rightCmdDeg, SERVO_RIGHT_REVERSE));
...
static void setWingIncidence(float symDeg, float diffDeg) {
  leftCmdDeg = SERVO_LEFT_NEUTRAL_DEG + symDeg - diffDeg;
  rightCmdDeg = SERVO_RIGHT_NEUTRAL_DEG + symDeg + diffDeg;
}
...
static void writeLogRow() {
  ...
  logFile.print(pwmFromServoDegrees(leftCmdDeg, SERVO_LEFT_REVERSE)); logFile.print(',');
  logFile.print(pwmFromServoDegrees(rightCmdDeg, SERVO_RIGHT_REVERSE)); logFile.print(',');
