...
static void updateFlightMode(uint32_t nowUs) {
  ...
  if (flightMode == MODE_CARRIED_FOLDED || modeStartUs == 0) {
    // Mode23 test starts at deployment transient and skips folded acceleration.
...
void setup() {
  ...
  serialMutex.lock();
  Serial.print(F("[Core 0] 626 Mode23 GlideEntryOnly boot, log tag="));
