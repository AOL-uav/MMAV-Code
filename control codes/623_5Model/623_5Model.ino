...
static const char LOG_TAG[] = "0623";
...
// Mission phase timing follows the latest M0-M5 table.
...
static const uint32_t MODE_FOLDED_ACCEL_MS = 1000;
static const uint32_t MODE_DEPLOYMENT_TRANSIENT_MS = 1000;
...
static uint32_t modeReleaseStartMs = 0;
...
static void updateFlightMode() {
  ...
  if (modeReleaseStartMs == 0) modeReleaseStartMs = millis();
  const uint32_t elapsedMs = millis() - modeReleaseStartMs;
  ...
  if (flightMode == MODE_AUTOROTATION_LANDING) {
    return;
  }
#endif

  if (elapsedMs < MODE_FOLDED_ACCEL_MS) {
    // M1: after release, keep wings folded and avoid active stabilization.
    flightMode = MODE_FOLDED_ACCELERATION;
    writeMorphServo(SERVO_MORPH_STOWED_DEG);
  } else if (elapsedMs <
             MODE_FOLDED_ACCEL_MS + MODE_DEPLOYMENT_TRANSIENT_MS) {
    // M2: sweep actuator moves wings forward; SMC remains inhibited.
    flightMode = MODE_DEPLOYMENT_TRANSIENT;
    writeMorphServo(SERVO_MORPH_DEPLOYED_DEG);
  } else {
    // M3 is the only active feedback phase in the current enabled code.
    flightMode = MODE_GLIDE_NAVIGATION;
    writeMorphServo(SERVO_MORPH_DEPLOYED_DEG);

#if ENABLE_AUTOROTATION_PHASES && ENABLE_GPS_PHASE_TRANSITIONS
    // Disabled future path: switch from M3 to M4 only after GPS/ESEKF local
    // position remains inside the autorotation entry region for several cycles.
    ...
    if (autorotationEntryConfirmCount >=
        AUTOROTATION_ENTRY_CONFIRM_CYCLES) {
      clearControlState();
      flightMode = MODE_AUTOROTATION_TRANSITION;
      autorotationModeStartMs = millis();
    }
#endif
  }
...
void setup() {
  ...
  serialMutex.lock();
  Serial.print(F("[Core 0] 623 5Model boot, log tag="));
  ...
  Serial.print(F("[Core 0] phase timing ms M1/M2="));
  Serial.print(MODE_FOLDED_ACCEL_MS);
  Serial.print(F("/"));
  Serial.println(MODE_DEPLOYMENT_TRANSIENT_MS);
