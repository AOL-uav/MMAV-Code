...
static 
static mutex_t serialMutex_m;

static void safeSerialPrintln(const __FlashStringHelper *s) {
  mutex_enter_blocking(&serialMutex_m);
  Serial.println(s);
  mutex_exit(&serialMutex_m);
}
...
char globalLogFileName[16] = "";
bool globalSdReady = false;

static void handleCommand(String cmd) {
  cmd.trim(); cmd.toLowerCase();
  if (cmd == "r") {
    if (globalSdReady && strlen(globalLogFileName) > 0) {
      mutex_enter_blocking(&serialMutex_m);
      Serial.print(F("# Dumping log: "));
      Serial.println(globalLogFileName);
      File file = sd.open(globalLogFileName, O_RDONLY);
      if (file) {
        ...
        Serial.println(F("# ERROR: Could not open log file for reading."));
      }
      mutex_exit(&serialMutex_m);
    } else {
      mutex_enter_blocking(&serialMutex_m);
      Serial.println(F("# ERROR: SD not ready or no file."));
      mutex_exit(&serialMutex_m);
    }
  }
}
  ...
  if (!sdReady) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (!SD.exists(filename)) {
        ...
        strncpy(globalLogFileName, filename, sizeof(globalLogFileName));
        break;
      }
    }
    if (logFile) {
...
static void maybeEnqueueLog(uint32_t controlDtUs, uint32_t controlExecUs, uint32_t esekfUs) {
  if (sweepMode != SWEEP_UNKNOWN) return;
  ...
  ControlRecord record;
  record.kind = RECORD_DATA;
  record.time_us = micros();
  record.dt_us = controlDtUs;
  record.exec_us = controlExecUs;
  record.esekf_us = esekfUs;
  record.roll_rad = rollRad;
  record.pitch_rad = pitchRad;
  record.yaw_rad = yawRad;
  record.pwm_morph = morphPwmUs;
  record.pwm_left = leftPwmUs;
  record.pwm_right = rightPwmUs;
  
  record.target_morph = currentSweepUs;
  record.target_left = currentAoaLeftUs;
  record.target_right = currentAoaRightUs;
