...
ControlRecord rb_data[QUEUE_DEPTH];
mutex_t rb_mutex;

bool rb_push(const ControlRecord* rec) {
  mutex_enter_blocking(&rb_mutex);
  int next = (rb_head + 1) % QUEUE_DEPTH;
  if (next == rb_tail) {
    mutex_exit(&rb_mutex);
    return false;
  }
...
static void handleCommand(String cmd) {
  ...
  if (cmd == "r") {
    if (globalSdReady && strlen(globalLogFileName) > 0) {
      ...
      FsFile file = sd.open(globalLogFileName, O_RDONLY);
      if (file) {
...
void setup1() {
  mutex_init(&serialMutex_m);
  mutex_init(&rb_mutex);
  delay(2000); 
  SPI.setRX(SD_MISO_PIN); SPI.setTX(SD_MOSI_PIN); SPI.setSCK(SD_SCK_PIN); SPI.begin();
  
  bool sdReady = sd.begin(SdSpiConfig(SD_CS_PIN, SHARED_SPI, SD_SCK_MHZ(12)));
  ...
  if (!sdReady) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (!sd.exists(filename)) {
        logFile = sd.open(filename, O_WRONLY | O_CREAT | O_APPEND);
        break;
      }
    }
    if (logFile) {
      ...
      globalLogFileName[sizeof(globalLogFileName)-1] = '\0';
...
bool takeFreeRecord(ControlRecord *&slot) {
  static ControlRecord scratch;
  slot = &scratch;
  return true;
}
