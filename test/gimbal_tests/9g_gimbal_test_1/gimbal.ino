...
const int minSafeUS = 1170; 
const int maxSafeUS = 1830;

void setup() {
  Serial.begin(115200);
  delay(2000);
  Serial.println("Gimbal v5.6: ALL AXES RE-ENABLED");
...
void attachAll() {
  if (!yawServo.attached()) yawServo.attach(yawPin, 800, 2200);
  if (!pitchServo.attached()) pitchServo.attach(pitchPin, 800, 2200);
  if (!rollServo.attached()) rollServo.attach(rollPin, 800, 2200);
  relaxed = false;
...
int degToUs(float deg) {
  return map(constrain(deg, 60, 120), 0, 180, 1000, 2000);
}
